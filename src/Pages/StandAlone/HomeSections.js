...
const HomeSections = () => {
  ...
  const handleOneDrop = (e) => {
    ...
    let section_order = sortedSections?.length
    CACHE_NUMBERS[section_order] = true;

    setSortedSection((prev) => [...prev, { ...currentSection, section_order }]);
    setSections((prev) =>
      prev?.filter((item) => item?.section_name !== currentSection?.section_name)
    );
  };
  ...
  const returnSortedSection = (item) => {
    ...
    setSortedSection((prev) =>
      prev?.filter((currentItem) => currentItem?.section_name !== item?.section_name)
    );
  ...
  const deleteSection = async (section) => {
    setSections(prev => prev?.filter(s => s?.section_name !== section?.section_name))
    if (section?.id) {
  ...
  return (
    <>
      ...
      <BlockPaper title="Home sections" headerClassName="flex items-center justify-between" contentBar={
        ...
        <div className="flex gap-8 flex-wrap md:flex-nowrap px-4">
          ...
          <div className="flex-1 min-w-[250px] ">
            ...
            >
              <ul
                className={`grid grid-cols-1 sm:grid-cols-2 ${sections?.length ? "" : "md:grid-cols-4"
                  } gap-4 p-4`}
              >
                {sortedSections?.map((section, index) => (
                  <li className="bg-white relative group-section  overflow-hidden border-gray-500 flex items-center shadow">
                    ...
                    />
                    <label htmlFor={section?.section_name} className="mx-auto flex items-center gap-2 whitespace-nowrap">
                      <input onChange={(e) => changeVisibility(e, section?.section_name)} id={section?.section_name} type="checkbox" name="display_home" defaultChecked={section?.display_home} value={section?.display_home} className="w-4 h-4" />
                      {section?.section_name}
                    </label>
                    <button
        ...
        <div className="flex gap-4 items-center p-4">
          ...
          <button onClick={onSaveOrder} className="bg-primary-blue text-white  p-2 px-8 rounded-md">
            Save Orders
          </button>
        </div>
      </BlockPaper>
