...
const SelectProduct = ({ tableName }) => {
  ...
  return (
    ...
    <div>
      <div className=" mb-4 -mt-4 flex gap-4 items-end justify-between bg-gray-100 -mx-5 shadow p-4">
        {
          tableName === 'sale' ? (
            <div className="relative">
              ...
              />
              {endDate ? (
                <span className="absolute top-0 right-0 bg-white pointer-events-none p-[6px] border-gray-200 border left-14 ml-1 flex items-center justify-between">
      ...
      <SelectedProductTable setProductLength={setProductLength} insertMany={handleChoose} tableName={tableName} selectedList={selectedList} setSelectedList={setSelectedList} id={id} />
    </div>
  );
};
